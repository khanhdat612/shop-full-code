...
const PageAdmin=() => {
 ...
 const  sendData2 ={
  ...
  //  valuePriceAccessory: data.productAccessory,
        ...
        axios.post('http://localhost/newCode/ANDROID/Add_product_ANDROID/Add_product.php', sendData2)
          .then((result) => {
  ...
  // 
   const [ product, setProduct ]=useState([]);
                useEffect(() => {
                    fetch("http://localhost/newCode/ANDROID/API_DATA_ANDROID/API_Android.php")
                    .then(res => res.json())
                    .then(
                        (result) => { 
                            setProduct(result);
   ...
   const [ product2, setProduct2 ]=useState([]);
                useEffect(() => {
                    fetch("http://localhost/newCode/ANDROID/API_DATA_ADDPRODUCT/API_Data_AddProduct.php")
                    .then(res => res.json())
                    .then(
                        (result) => { 
                            setProduct2(result);
  ...
  const deleteProduct = (id) => {
    axios.delete('http://localhost/newCode/ANDROID/DELETE_PRODUCT_ADD/Delete.php', { data: { id: id } })
      .then((result) => { 
        product2();
      }).catch(() => {
    ...
    return (
        <div>
            ...
            <h1>DANH SÁCH SẢN PHẨM ANDROID</h1>
                ...
                {product2.map((data, index) => {
                  return (
                          <table className='table table-1 table-2' key={index}>
                            <tr className='tr'>
                                <td className='td td-1'>{ data.fullName }</td>
                                <td className='td td-2'>{ data.price}</td>
                                <td className='td td-2'>{ data.pricesale}</td>
                        <td className='td td-2'>{data.quantity}</td>
                        <td style={{ display: "flex" }}>
                          <Link className='Delete' to="" onClick={() => deleteProduct(data.id)}>
                        XÓA
                          </Link>
                        </td>
                            </tr>
